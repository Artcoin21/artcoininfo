      ...
      let req = this.#protocol.request(options, res => {
        ...
        res.on('end', () => {
          if (res.statusCode === 401) {
            reject(new Error(`Tachacoin JSON-RPC: Connection Rejected: 401 Unauthorized`))
          } else if (res.statusCode === 403) {
            reject(new Error(`Tachacoin JSON-RPC: Connection Rejected: 403 Forbidden`))
          } else if (res.statusCode === 500 && buffer === 'Work queue depth exceeded') {
            let exceededError = new Error(`Tachacoin JSON-RPC: ${buffer}`)
            ...
            try {
              ...
              reject(new Error(`Tachacoin JSON-RPC: Error Parsing JSON: ${err.message}`))
            }
          }
        })
      })
      req.on('error', err => reject(new Error(`Tachacoin JSON-RPC: Request Error: ${err.message}`)))
